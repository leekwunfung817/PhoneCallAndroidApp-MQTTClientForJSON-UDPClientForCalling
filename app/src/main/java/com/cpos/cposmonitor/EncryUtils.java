...
public class EncryUtils {
    ...
    public static EncryUtils getInstance(@NonNull final Context context) {
        synchronized (EncryUtils.class) {
            if (null == encryUtilsInstance) {
                encryUtilsInstance = new EncryUtils(context);
            }
        }
        return encryUtilsInstance;
    }

    public EncryUtils(@NonNull final Context context) {
        ...
        if(!"".equals(alias)){
            try {
                ...
                if (!keyStore.containsAlias(alias)) {
                    ...
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR2) {
                        spec = new KeyPairGeneratorSpec.Builder(this.context)
                                .setAlias(alias)
